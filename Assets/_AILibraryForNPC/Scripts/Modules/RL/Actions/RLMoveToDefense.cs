...
public class MoveToDefense : BaseAction
{
    ...
    public override void PrePerform(WorldState worldState)
    {
        ...
        currentTarget = (worldState as BarbarianWorldState).targetDefense;
        if (currentTarget != null)
        {
            agent.SetDestination(currentTarget.transform.position);
            Debug.LogWarning("MoveToDefense: " + currentTarget.name);

            (worldState as BarbarianWorldState).reward -= 2;
        }
    }
}
