...
namespace GraphQlClient.Core
{
    ...
    public class GraphApi : ScriptableObject
    {
        ...
        public async Task<ClientWebSocket> Subscribe(
            Query query,
            string socketId = "1",
        ...
        )
        {
            if (string.IsNullOrEmpty(query.query))
                query.CompleteQuery();
            return await HttpHandler.WebsocketConnect(
                url,
                query.query,
                authToken,
                socketId,
        ...
        {
            loading = true;
            request = await HttpHandler.PostAsync(
                url,
                Introspection.schemaIntrospectionQuery,
                authToken
            );
        ...
        void HandleIntrospection()
        {
            ...
            File.WriteAllText(
                Application.dataPath + $"{""}{name}schema.txt",
                introspection
            );
        ...
        public void GetSchema()
        {
            if (schemaClass == null)
            {
                try
                {
                    introspection = File.ReadAllText(
                        Application.dataPath + $"{""}{name}schema.txt"
                    );
                }
                catch
                ...
                schemaClass = JsonConvert.DeserializeObject<Introspection.SchemaClass>(
                    introspection
                );
                if (schemaClass.data.__schema.queryType != null)
                    queryEndpoint = schemaClass.data.__schema.queryType.name;
        ...
        public void CreateNewMutation()
        {
            ...
            Introspection.SchemaClass.Data.Schema.Type mutationType =
                schemaClass.data.__schema.types.Find((aType => aType.name == mutationEndpoint));
        ...
        public void CreateNewSubscription()
        {
            ...
            Introspection.SchemaClass.Data.Schema.Type subscriptionType =
                schemaClass.data.__schema.types.Find((aType => aType.name == subscriptionEndpoint));
        ...
        {
            Introspection.SchemaClass.Data.Schema.Type type = schemaClass.data.__schema.types.Find(
                (aType => aType.name == typeName)
            );
        ...
        public void GetQueryReturnType(Query query, string queryName)
        {
            ...
            Introspection.SchemaClass.Data.Schema.Type.Field field = queryType.fields.Find(
                (aField => aField.name == queryName)
            );
            ...
            public void CompleteQuery()
            {
                ...
                for (int i = 0; i < fields.Count; i++)
                {
                    ...
                    if (field.parentIndexes.Count == 0)
                    {
                        ...
                        {
                            int count =
                                previousField.parentIndexes.Count - field.parentIndexes.Count;
                            while (count > 0)
                            {
        ...
        public class Field
        {
            ...
            {
                Introspection.SchemaClass.Data.Schema.Type t = schemaClass.data.__schema.types.Find(
                    (aType => aType.name == type)
                );
            ...
            public static explicit operator Field(
                Introspection.SchemaClass.Data.Schema.Type.Field schemaField
            )
            {
